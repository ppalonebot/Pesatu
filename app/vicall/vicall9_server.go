...
func (me *JSONSignal) Handle(send chan []byte, req *jsonrpc2.RPCRequest) {
	replyError := func(err error) {
		...
		if err != nil {
			me.Logger.Error(err, "error while sending reply with error")
			return
		}
	...
	me.Logger.V(2).Info(fmt.Sprintf("On req method %s, id: %s", req.Method, req.ID))
	...
	case "join":
		...
		if err != nil {
			me.Logger.Error(err, "connect: error parsing offer")
			replyError(err)
			break
		}

		me.OnOffer = func(offer *webrtc.SessionDescription) {
			me.Logger.V(2).Info(fmt.Sprintf("Notify to peer_id: %s, sdp:%s", me.ID(), offer.SDP))
			rpcReq, err := jsonrpc2.Notify("offer", offer)
			if err != nil {
				me.Logger.Error(err, "error sending offer")
				return
			}
		...
		me.OnIceCandidate = func(candidate *webrtc.ICECandidateInit, target int) {
			...
			if err != nil {
				me.Logger.Error(err, "error sending ice candidate")
				return
			}
		...
		me.OnICEConnectionStateChange = func(s webrtc.ICEConnectionState) {
			me.Logger.V(2).Info(fmt.Sprintf("peer_id: %s OnICEConnectionStateChange! %s", me.ID(), s.String()))
		}
		...
		me.Logger.V(2).Info(fmt.Sprintf("Reply join to req id:%s, with peer_id: %s@%s, sdp:%s", req.ID, me.ID(), join.SID, answer.SDP))
	...
	case "offer":
		...
		me.Logger.V(2).Info(fmt.Sprintf("Reply offer to req id:%s, with answer sdp:%s", req.ID, answer.SDP))
		resRpc, err := jsonrpc2.Reply(req.ID, answer)
		if err != nil {
	...
	case "answer":
		...
		err := json.Unmarshal(req.Params, &negotiation)
		if err != nil {
			me.Logger.Error(err, "connect: error parsing answer")
			replyError(err)
			break
		}
	...
	case "trickle":
		...
		if err != nil {
			me.Logger.Error(err, "connect: error parsing candidate")
			replyError(err)
			break
		}
	...
	case "dmessage":
		...
		if err != nil {
			...
			me.Logger.V(2).Info(fmt.Sprintf("sending %s", string(jsonBytes)))
